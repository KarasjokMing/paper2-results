...
PARAM_PATH = os.path.join(os.path.dirname(__file__), "optimal_params.json")
...
fig, axes = plt.subplots(2, 3, figsize=(7.4, 5.0), constrained_layout=True)
last_im = None
for ax, (ch, xname, yname, xr, yr, title) in zip(axes.ravel(), PLOTS):
...
for ext in ("pdf", "png"):
    fig.savefig(os.path.join(OUT_DIR, f"fig_phase_routing_maps.{ext}"))
plt.close(fig)
