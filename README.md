...
- `src/direct_solver_6x6.py`: direct 14x14 solver and 6x6 scattering matrix construction.
...
- `figures/fig2_representative_maps.pdf`: main-text Fig. 2 with exactly `T1`, `R1`, `T2`, `T3`.
- `figures/fig2_all6_supplementary.pdf`: all-six supplementary/repository map containing `T1`, `R1`, `T2`, `R2`, `T3`, `R3`.
...
- Eq. `S_dir = P A^{-1} B`: `src/direct_solver_6x6.py`.
...
`python/deprecated/oracle_compact.py` is retained only for provenance. It constructs the old symmetry-completed matrix using `S=S^T` and waveguide reversal. It is not the exact 6x6 scattering matrix and must not be used as the truth source. Use `src/direct_solver_6x6.py` instead.
