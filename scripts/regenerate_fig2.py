...
def t1_sufficient_map(xname, yname, xr, yr, n=280):
    """Scan unrelated phases while imposing P_{1+}=0 for the T1 route."""
    ci = CHANNELS.index("T1")
    xi = PARAM_NAMES.index(xname)
    yi = PARAM_NAMES.index(yname)
    base = vector_for("T1")

    # In pi-normalized units, P_{1+}=0 means phi1 + theta_a1 = 1 mod 2.
    base[PARAM_NAMES.index("phi1")] = 0.25
    base[PARAM_NAMES.index("theta_a1")] = 0.75
...
PLOTS_MAIN = [
    ("T1S", "phi2", "theta_b2", (0, 2), (0, 2), r"$T_1$: $P_{1+}=0$ imposed"),
    PLOTS_ALL6[1],
    PLOTS_ALL6[2],
    PLOTS_ALL6[4],
]
...
def render_panel(ax, ch, xname, yname, xr, yr, title):
    n = 360 if ch in ("T3", "R3") else 280
    if ch == "T1S":
        xs, ys, grid, xo, yo = t1_sufficient_map(xname, yname, xr, yr, n=n)
    else:
...
def draw_maps(plots, shape, figsize, basename, aliases=()):
    ...
    for ax, (ch, xname, yname, xr, yr, title) in zip(axes_flat, plots):
        last_im = render_panel(ax, ch, xname, yname, xr, yr, title)
...
def draw_t1_sufficient():
    ...
    im = render_panel(
        ...
        (0, 2),
        (0, 2),
        r"$T_1$: $P_{1+}=0$ imposed",
    )
    cbar = fig.colorbar(im, ax=ax, shrink=0.92, pad=0.02)
    cbar.set_label("target probability")
    cbar.ax.tick_params(labelsize=7)
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(OUT_DIR, f"t1_sufficient_condition_scan.{ext}"))
    plt.close(fig)


draw_maps(PLOTS_MAIN, (2, 2), (6.2, 5.0), "figure2_representative", aliases=("fig2_representative_maps",))
draw_maps(PLOTS_ALL6, (2, 3), (7.4, 5.0), "figure2_all6_supplementary", aliases=("fig2_all6_supplementary",))
draw_t1_sufficient()
