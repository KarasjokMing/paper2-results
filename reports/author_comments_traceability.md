...
| 7. Old branch/phase scatter plot | Done | No disconnected old branch/scatter plot was added to the main manuscript. | This report records the decision: rejected for the main text because it did not answer a clear physical question beyond the continuous correlated-branch Fig. 3. | If desired later, it should live only in repository or supplement and be continuous, not used as proof. |
...
| 13. \(C_2(q)\) appears only in correlated-branch subsection | Done | Section III.A discusses Fig. 2 qualitatively; \(\mathcal{C}_2(q)\) first appears in the correlated-branch subsection at `manuscript/main.tex:400`. | Search confirms no literal `\mathcal{C}_2(q)` before the correlated-branch subsection. | Section III.A still refers qualitatively to a correlated branch neighborhood introduced below, without defining \(C_2(q)\). |
