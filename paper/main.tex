...
\begin{abstract}
We analyze single-photon scattering in a waveguide-QED node formed by two giant atoms coupled to three one-dimensional waveguides.
The two atoms connect three waveguides, so a photon entering from either side of any waveguide can leave through six output ports.
Using a real-space scattering formulation, we derive the single-photon amplitudes by solving the full set of boundary and atomic equations.
The resulting $6\times6$ scattering matrix is built from six independently solved incident columns and is unitary to numerical precision.
Local coupling phases provide independent interference knobs that are absent in the corresponding small-atom node; by tuning them, selected output channels can be enhanced while the other five are suppressed.
The phase-reversed device is the time-reversal partner of the original one, giving the probability relation $T_{ij}(\theta)=T_{ji}(-\theta)$ rather than the ordinary reciprocity condition $S=S^{\mathsf T}$.
Representative fixed-backbone parameter sets show near-unit single-photon routing probabilities and extended high-fidelity basins in phase space.
\end{abstract}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig1_system_schematic_vector_cropped.pdf}
  \caption{Two-giant-atom, three-waveguide six-port node. Atom $a$ couples to waveguides~1 and~2, while atom $b$ couples to waveguides~2 and~3. For WG1-L incidence, the output amplitudes are $r_1$ (port~1), $t_1$ (port~2), $r_2$ (port~3), $t_2$ (port~4), $r_3$ (port~5), and $t_3$ (port~6).}
  \label{fig:model}
\end{figure}
...
t_1 &= 1 - \ii\eta_a^{(1)} P_{1+}^*, \label{eq:t1_eta}\\[2pt]
r_2 &= -\ii\bigl(\eta_a^{(1)} P_{2-}^a + \eta_b^{(1)} P_{2-}^b\bigr), \label{eq:r2_eta}\\[2pt]
t_2 &= -\ii\bigl(\eta_a^{(1)} P_{2+}^{a*} + \eta_b^{(1)} P_{2+}^{b*}\bigr), \label{eq:t2_eta}\\[2pt]
r_3 &= -\ii\eta_b^{(1)} P_{3-}, \label{eq:r3_eta}\\[2pt]
t_3 &= -\ii\eta_b^{(1)} P_{3+}^*. \label{eq:t3_eta}
\end{align}
\end{subequations}
Here we have introduced a systematic phase-factor notation to distinguish output directions:
\begin{equation}
P_{j,m}^{L} = 1+\ee^{\ii(\ph_j-\theta_{mj})},\quad
P_{j,m}^{R} = 1+\ee^{-\ii(\ph_j+\theta_{mj})},
\label{eq:Pnotation_wg1}
\end{equation}
which for waveguides~1 and~3 specialize to $P_{1-}\equiv P_{1,a}^{L}$, $P_{1+}^*\equiv P_{1,a}^{R}$, $P_{3-}\equiv P_{3,b}^{L}$, $P_{3+}^*\equiv P_{3,b}^{R}$.
...
\end{align}
\end{subequations}
where $N_{t_1}$ is given in Appendix~\ref{app:t1}. The remaining amplitudes $t_2$, $t_3$, $r_3$ do not admit comparably simple elimination and are most compactly represented in the $\eta$-form of Eqs.~\eqref{eq:t2_eta}--\eqref{eq:t3_eta}.
...
Z(t_2)&: \eta_a^{(1)}P_{2+}^{a*} + \eta_b^{(1)}P_{2+}^{b*}=0,\\
Z(r_2)&: \eta_a^{(1)}P_{2-}^{a} + \eta_b^{(1)}P_{2-}^{b}=0,\\
Z(t_3)&: \Poneplus=0 \;\text{or}\; P_{3+}^*=0,\\
...
\begin{table}[!htbp]
\caption{Representative sufficient zero-condition coverings for waveguide-1 edge incidence. Conditions $C_2^t$ and $C_2^r$ denote the linear-combination zeros $\eta_a^{(1)}P_{2+}^{a*}+\eta_b^{(1)}P_{2+}^{b*}=0$ and $\eta_a^{(1)}P_{2-}^{a}+\eta_b^{(1)}P_{2-}^{b}=0$, respectively. The table gives constructive coverings used in this work and is not a necessary-and-sufficient classification of all mathematical solutions.}
...
$T_3$ & $N_{t_1}=0,\ \Poneminus=0,\ C_2^t=0,\ C_2^r=0,\ P_{3-}=0$ & $P_{3+}^*\neq0,\ \Poneplus\neq0,\ M\neq0$ & Near-resonant representative route \\
$R_3$ & $N_{t_1}=0,\ \Poneminus=0,\ C_2^t=0,\ C_2^r=0,\ P_{3+}^*=0$ & $P_{3-}\neq0,\ \Poneplus\neq0,\ M\neq0$ & Near-resonant representative route \\
\end{tabular}%
}
\end{table}
...
It requires entrance locking via $\Poneminus=0$ (since $\Poneplus=0$ would also kill the target), waveguide-2 closure via $C_2^t=C_2^r=0$, and direction selectivity on waveguide~3 through $P_{3+}^*=0$ or $P_{3-}=0$.
...
The plotted coordinates are chosen channel by channel so that the relevant structure is visible: broad routes are shown on full or seam-centered phase windows, while the remote waveguide-3 routes require a magnified near-resonant window.
All panels use the same probability color scale, so the areas of the high-fidelity regions can be compared directly.
The remaining parameters are held at their optimized values listed in Appendix~\ref{app:opt}.

\begin{figure*}[!htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{fig_phase_routing_maps.pdf}
  \caption{Phase-coordinate routing landscapes around the six fixed-backbone optima for waveguide-1 edge incidence.
  The $T_1$ panel shows the full $0$--$2\pi$ phase plane, while the $R_1$ panel is recentered at the periodic seam so that the two edge features join into one continuous high-transmission region.
  The $T_2$ and $R_2$ panels use extended branch windows large enough to show the boundary of the high-fidelity islands around the correlated branch neighborhoods introduced below.
  The $T_3$ and $R_3$ panels use magnified near-resonant windows because their high-fidelity regions are much narrower on the full phase torus.
  Contours mark $P=0.9$, $0.99$, and $0.999$ where visible.
  These two-dimensional cuts demonstrate sufficient high-probability routes around representative optima; they are not intended as uniqueness proofs for the corresponding routing conditions.
  The exact optimized parameters are given in Table~\ref{tab:opt}.}
  \label{fig:phase_maps}
\end{figure*}

The $T_1$ panel is particularly instructive: the broad diagonal band of high transmission directly visualizes the sufficient direct phase-zero condition $\ph_1+\thaone=(2n+1)\pi$ ($\Poneplus=0$), confirming that the entire one-parameter family of phase combinations on this line achieves near-unit routing.
...
The width of the band reflects the insensitivity of the denominator $M$ to the specific value of the phase sum within a reasonable tolerance.

The $T_2$ and $R_2$ panels exhibit compact high-probability islands, consistent with the composite-interference mechanism: these channels require the simultaneous satisfaction of multiple zero conditions, leaving a lower-dimensional solution manifold.
Both are shown because the two output directions correspond to different linear-combination zeros, even though they are symmetry-related.
The $T_3$ and $R_3$ panels show very narrow near-resonant ridges, reflecting the singular nature of the waveguide-3 routing mechanism discussed in Sec.~\ref{sec:edge}. These channels require sub-percent-level tuning of the coupling phases.

\subsection{Correlated branch for \texorpdfstring{$T_2/R_2$}{T2/R2} routing}

The compact $T_2$ and $R_2$ islands in Fig.~\ref{fig:phase_maps} become more transparent when the correlated phase branch behind the composite-interference route is imposed explicitly.
...
Figure~\ref{fig:branch_verification} makes the role of the correlated $T_2/R_2$ phase relations explicit.
For each channel we compare two scans on the same $(q,\eta)$ window and with the same probability color scale.
In the unconstrained scan, only the local variables $(q,\eta)$ are varied around a single high-transmission point, while the remaining phases are held fixed.
In the constrained scan, the full branch $\mathcal{C}_2(q)$ is imposed and the same transverse deviation $\eta$ is scanned.
The high-fidelity area is then much larger, showing that the phase relations are not merely a convenient parametrization but actively enlarge the routing basin.

\begin{figure*}[!htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{fig_constructive_branch_verification.pdf}
  \caption{Effect of enforcing the constructive $T_2/R_2$ phase branch.
  Left column: only the two local variables $(q,\eta)$ are scanned around a single high-transmission point, without enforcing the correlated relations in $\mathcal{C}_2(q)$.
  The insets magnify the narrow high-fidelity spots in the unconstrained scans.
  Right column: the full $\mathcal{C}_2(q)$ branch is imposed and the same transverse deviation $\eta$ is scanned.
  All panels use the same color scale; contours mark $P=0.9$, $0.99$, and $0.999$.
  The displayed area fractions $A_{.99}$ and $A_{.999}$ show that the derived phase constraints substantially enlarge the high-fidelity basin.}
...
\begin{equation}
  t_3 \propto P_{3+}^* = 1+\ee^{-\ii(\ph_3+\thbthree)},\qquad
...
The verification code is organized in the online repository prepared at \url{https://github.com/KarasjokMing/paper2-results} and will be made public upon submission or publication.

\section{Optimized routing parameters}
\label{app:opt}

Table~\ref{tab:opt} lists the independently optimized parameter sets for waveguide-1 edge incidence that achieve the routing probabilities shown in Fig.~\ref{fig:phase_maps}.
